...
return [
    // prefix used for admin panel backend routes
    'backend_prefix' => env('DELTA_BACKEND_PREFIX', 'api/backend'),

    // if true - generated urls will end with /
    'trailing_slash' => env('TRAILING_SLASH', false),

    // image settings for delta ImageRule
    'images' => [
        'types' => ['png', 'jpg', 'webp'],
        'size' => 10 * 1024,
        'max_width' => 2048,
    ...
    'server_type' => env('DELTA_SERVER_TYPE', 'nginx'), // nginx or apache but apache is not supported yet

     // block page indexing in development mode
    'noindex-nofollow' => env('DELTA_NOINDEX_NOFOLLOW', env('APP_ENV') !== 'production'),

    // highest priority domain which used first
    'highest_priority_frontend_domain' => env('STATIC_FRONTEND_DOMAIN', false),

    // if false DELTA_DEFAULT_FRONTEND_DOMAIN will take as a current domain, else - domain will take from request
    'domain_from_referer' => env('DELTA_DOMAIN_FROM_REFERER', false),

    // default domain which used if 'domain_from_referer' = false
    'default_frontend_domain' => env('DELTA_DEFAULT_FRONTEND_DOMAIN', env('APP_URL')),

    // allow requests from this IPs
    'excepted_ip_referrers' => explode(',', env('DELTA_EXCEPTED_IP_REFERRERS', '::1,127.0.0.1')),

    // creation of pages with system urls will be blocked
    'system_urls' => [
        // 'options/layout', 'user'
    ],

    // basic delta models can be replaced with custom models
    'models' => [
        'user' => "\App\Models\User",
        'domain' => \CfDigital\Delta\Core\Models\Domain::class,
        'page' => \CfDigital\Delta\Core\Models\Page::class,
    ],

    // replace delta page templates with custom templates
    'otherClasses' => [
        'homepage_pageTemplate' => '\App\PageTemplates\HomepageTemplate',
        'page_template' => '\App\PageTemplates\PageTemplate',
    ],

    // add separate module type in editor modules list
    'editorElements' => [
        //    'news' => \CfDigital\Delta\Core\Services\EditorElements\NewsEditorElement::class,
        'infoblocks' => \CfDigital\Delta\Core\Services\EditorElements\InfoBlockEditorElement::class,
        'forms' => \CfDigital\Delta\Core\Services\EditorElements\FormEditorElement::class,
    ],

    // used for creation models permissions in base DatabaseSeeder
    'permissions' => [
        'crud' => [
            'role', 'user', 'page', 'infoblock', 'form',
            'field', 'domain', 'structure', 'redirect', 'metatag', 'setting', 'translation', 'menu',
            'rich_generator', 'template_styles'
        ],
        'other' => [
    ...
    // app supported locales. Locale listed first will be a main app locale
    'supportedLocales' => [
